...
public class HallowedSepulchreOverlay extends Overlay
{
    ...
    {
        if (!config.highlightTeleports())
        {
            return;
        }

        for (HallowedSepulchreTeleport teleport : plugin.getTeleports()) {
            final Color color = (teleport.getTicksUntilDespawn() == 0 ? Color.RED : config.teleportTickColor());
            GraphicsObject graphicsObject = teleport.getGraphicsObject();
            final String ticksLeftStr = String.valueOf(teleport.getTicksUntilDespawn());

            Polygon poly = Perspective.getCanvasTilePoly(client, graphicsObject.getLocation());
            if (poly != null)
            {
                OverlayUtil.renderPolygon(graphics2D, poly, config.teleportTileColor());
            }
            Point canvasPoint = Perspective.getCanvasTextLocation(client, graphics2D, graphicsObject.getLocation(), "X", 0);

            if (teleport.isSpawning()) {
                if (canvasPoint != null)
                {
                    renderTextLocation(graphics2D, canvasPoint, "4", color);
                }

            } else {
                if (canvasPoint != null)
                {
                    renderTextLocation(graphics2D, canvasPoint, ticksLeftStr, color);
                }
            }
        }
    }
