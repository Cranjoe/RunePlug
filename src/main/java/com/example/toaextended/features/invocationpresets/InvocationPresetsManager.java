...
public class InvocationPresetsManager implements PluginLifecycleComponent
{
	...
	public void addPreset(InvocationPreset preset)
	{
		...
		configManager.setConfiguration(ToaExtendedConfig.CONFIG_GROUP, CONFIG_KEY_PRESETS + "." + preset.getName(), preset.serialize());
	...
	private void deletePreset(InvocationPreset preset)
	{
		...
		configManager.unsetConfiguration(ToaExtendedConfig.CONFIG_GROUP, CONFIG_KEY_PRESETS + "." + preset.getName());
	...
	{
		for (String key : configManager.getConfigurationKeys(ToaExtendedConfig.CONFIG_GROUP + "." + CONFIG_KEY_PRESETS))
		{
			try
			{
				String keySuffix = key.split("\\.", 3)[2];
				String configValue = configManager.getConfiguration(ToaExtendedConfig.CONFIG_GROUP, CONFIG_KEY_PRESETS + "." + keySuffix);
