...
public class DemonicGorillaOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
        for (DemonicGorilla gorilla : this.plugin.getGorillas().values()) {
            ...
            if (lp != null) {
                //Point point = Perspective.localToCanvas(this.client, lp, this.client.getTopLevelWorldView().getPlane(), -48);
                Point point = Perspective.localToCanvas(this.client, lp, client.getTopLevelWorldView().getPlane(), -48);
                if (point != null) {
